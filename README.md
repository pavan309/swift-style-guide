...
This guide was last updated for Swift 4.0 on August 17, 2018.
...
* **1.2** Avoid uncomfortably long lines with a hard maximum of 100 characters per line (Xcode->Preferences->Text Editing->Page guide at column: 100 is helpful for this)
...
* **3.1.12** When using a statement such as `else`, `catch`, etc. that follows a block, put this keyword on the same line as the block. Again, we are following the [1TBS style](https://en.m.wikipedia.org/wiki/Indentation_style#1TBS) here. Example `if`/`else` and `do`/`catch` code is below.
...
* **4.1.1** 100 character column limit (like the rest of the code).
